    ...
    if ($action === 'update_photo') {
        ...
        if (isset($_FILES['foto']) && $_FILES['foto']['error'] === 0) {
            ...
            if (in_array($ext, $allowed)) {
                ...
                $foto = 'admin_' . $_SESSION['user_id'] . '_' . time() . '.' . $ext;
                ...
                if ($stmt->execute([$foto, $_SESSION['user_id']])) {
                    $_SESSION['foto'] = $foto;
                    logAktivitas($pdo, $_SESSION['user_id'], 'Update Foto Profil', 'Mengupdate foto profil admin');
        ...
        if (!password_verify($current_password, $user['password'])) {
            ...
            if ($stmt->execute([$hashedPassword, $_SESSION['user_id']])) {
                logAktivitas($pdo, $_SESSION['user_id'], 'Ganti Password', 'Mengganti password akun admin');
...
<head>
    ...
    <title>Profil Admin - Marvell Rental</title>
    ...
    <div class="dashboard">
        ...
        <main class="main-content">
            <div class="dashboard-header">
                <h1 class="dashboard-title">Profil Admin</h1>
            </div>
                ...
                <div style="flex: 1;">
                        ...
                        <form method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <div style="position: relative;">
                                    <input type="password" name="current_password" id="admin_current_pw" class="form-control" required style="padding-right: 45px;">
                                    <button type="button" onclick="togglePassword('admin_current_pw', 'adminToggle1')" style="position: absolute; right: 10px; top: 50%; transform: translateY(-50%); background: none; border: none; color: var(--text-secondary); cursor: pointer; padding: 5px;">
                                        <i class="fas fa-eye" id="adminToggle1"></i>
                                    </button>
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <div style="position: relative;">
                                    <input type="password" name="new_password" id="admin_new_pw" class="form-control" required minlength="6" style="padding-right: 45px;">
                                    <button type="button" onclick="togglePassword('admin_new_pw', 'adminToggle2')" style="position: absolute; right: 10px; top: 50%; transform: translateY(-50%); background: none; border: none; color: var(--text-secondary); cursor: pointer; padding: 5px;">
                                        <i class="fas fa-eye" id="adminToggle2"></i>
                                    </button>
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <div style="position: relative;">
                                    <input type="password" name="confirm_password" id="admin_confirm_pw" class="form-control" required style="padding-right: 45px;">
                                    <button type="button" onclick="togglePassword('admin_confirm_pw', 'adminToggle3')" style="position: absolute; right: 10px; top: 50%; transform: translateY(-50%); background: none; border: none; color: var(--text-secondary); cursor: pointer; padding: 5px;">
                                        <i class="fas fa-eye" id="adminToggle3"></i>
                                    </button>
                                </div>
                            </div>
                    ...
                    <div style="margin-top: 20px; padding: 20px; background: rgba(255, 215, 0, 0.1); border-radius: 10px;">
                        ...
                        <p style="font-size: 0.85rem; color: var(--text-secondary); line-height: 1.6;">
                            Sebagai admin, Anda hanya dapat mengubah foto profil dan password. Untuk mengubah data lainnya, silakan hubungi system administrator.
                        </p>
                    </div>
                </div>
            </div>
